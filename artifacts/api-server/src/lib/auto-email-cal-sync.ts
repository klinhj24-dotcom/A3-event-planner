...
export async function syncAutoEmailsToCalendar(): Promise<void> {
  ...
  const rows = await db.execute(sql`SELECT auto_email_id, calendar_event_id, was_sent FROM auto_email_cal_events`);
  const existing = new Map<string, { calEventId: string; wasSent: boolean }>();
  for (const r of rows.rows as any[]) {
    existing.set(r.auto_email_id, { calEventId: r.calendar_event_id, wasSent: r.was_sent });
  }
  ...
  for (const entry of entries) {
    ...
    if (!rec) {
      ...
      try {
        ...
        if (result.data.id) {
          await db.execute(sql`
            INSERT INTO auto_email_cal_events (auto_email_id, calendar_event_id, was_sent)
            VALUES (${entry.id}, ${result.data.id}, ${entry.sent})
    ...
    } else if (rec.wasSent !== entry.sent) {
      // Sent status changed — update summary + description
      try {
        ...
        await db.execute(sql`
          UPDATE auto_email_cal_events SET was_sent = ${entry.sent} WHERE auto_email_id = ${entry.id}
        `);
        updated++;
      } catch (err: any) {
