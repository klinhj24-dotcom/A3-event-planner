...
- **Events**: Track events (student showcases, community events, recitals, open mics). Financial tracking (cost/revenue, paid/unpaid). Push events to Google Calendar. Calendar view (List/Calendar toggle, month grid with color-coded event pills). Per-event feature flags: Band Lineup (`hasBandLineup`), Staff Schedule (`hasStaffSchedule`), Call Sheet (`hasCallSheet`), Packing List (`hasPackingList`), Lead Generating (`isLeadGenerating`). Flags are auto-populated from event type defaults when creating a new event; overrideable individually.
- **Event Type Feature Defaults**: Each event type in Settings → Event Types has 4 toggleable feature pills (Band Lineup, Staff Schedule, Call Sheet, Packing List). These default flags are inherited when creating a new event of that type. Action buttons on event cards are gated behind their respective flags (except Send Invite, which is always shown).
- **Staff Pay in Financials**: Staff slots on events track bonus pay per slot (`bonusPay` decimal). Staff pay total = Σ(hours worked × employee hourly rate + bonus). Displayed as a cost line item in the event detail financials panel and factored into the net figure in both detail and list views. The list endpoint computes `staffPayTotal` per event via a single aggregate query.
- **Post-Event Debrief enhancements**: Debrief sheet now includes `leadsCollected`, `trialSignups`, `eventVibe` (energy rating dropdown), and `staffNotes` fields. The first three are conditionally shown in a "Lead Results" section only when the event has `isLeadGenerating` enabled; `staffNotes` is always shown.
...
- **Performer Guest List**: Per-ticketed-event toggle (`allowGuestList`). Policy: students_only / plus_one / plus_two. Auto-generates entries from lineup band members. Each entry gets a unique public link (`/guest-list/:token`) sent to the primary contact. Parents pre-register guest names. Admin can add manual VIP entries. Door check-in toggles per person (Student / Guest 1 / Guest 2). Schema: `eventGuestListTable` in `lib/db/src/schema/events.ts`. Routes in `artifacts/api-server/src/routes/guest-list.ts`. Public form at `artifacts/studio-hub/src/pages/guest-list.tsx`.
...
- **Post-Event Debrief — `hasDebrief` flag**: Events can enable a "Post-Event Debrief" toggle (in both create/edit forms). The debrief button on event cards and in the overview sheet is gated behind `event.hasDebrief`. `isLeadGenerating` is a separate independent flag.
- **Debrief access control**: Only the assigned `primaryStaffId` owner can fill out the debrief form. Everyone else sees a read-only view or "not yet submitted" placeholder. Owner match only — no admin bypass.
- **Debrief nudge cron**: Hourly cron (`lib/debrief-reminders.ts`) emails the debrief owner for events that have ended, have `hasDebrief=true`, have a `primaryStaffId`, and no submitted debrief. Marks `debriefNudgeSent=true` to prevent repeat nudges.
- **Dashboard — Pending Debriefs card**: Teal card at the top of the dashboard shown to the debrief owner for events ending within the ±window where no debrief exists yet.
- **Dashboard — Recent Debriefs card**: Violet card showing the last 10 submitted debriefs (last 14 days), with submitter name and date. Visible to all staff.
- **`canViewFinances` access control**: New boolean on `users` (default false). When false: Financials section hidden from event create/edit forms; PAID/UNPAID badge and net figures hidden in the event list table and overview panel; entire Payroll page shows an "Access Required" gate. Finance toggle per portal user is in Employees page (admin-visible switch below portal role).
- **Reports page** (`/reports`): Historical event data — summary stat cards (total events, lead events, total revenue/cost gated by canViewFinances), bar charts for events per month and revenue per month, monthly breakdown table, and event-type breakdown. Linked in the sidebar.
- **CSV export**: Export buttons on Contacts, Events, and Payroll pages. Events and Payroll CSV exports omit financial columns for users without `canViewFinances`.
