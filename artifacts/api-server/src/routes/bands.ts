...
router.post("/events/:id/lineup/auto-sort", async (req, res) => {
  ...
  try {
    ...
    for (const s of actSlots) {
      ...
      // NOTE: Skip reasons written by Smart Sort itself ("Can't accommodate:...")
      // to avoid a circular reference on the next run.
      if (s.scheduleConflict && notes.length === 0) {
        const reason = s.conflictReason?.trim();
        if (reason && !/^can't accommodate/i.test(reason)) {
