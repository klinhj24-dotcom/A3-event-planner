...
router.post("/employees", async (req, res) => {
  ...
  try {
    const { name, email, phone, role, isActive, notes } = req.body;
    if (!name || !role) {
    ...
    const [employee] = await db
      ...
      .values({ name, email, phone, role, isActive: isActive ?? true, notes })
      .returning();
...
router.put("/employees/:id", async (req, res) => {
  ...
  try {
    ...
    const [employee] = await db
      ...
      .set({ name, email, phone, role, isActive, notes, updatedAt: new Date() })
