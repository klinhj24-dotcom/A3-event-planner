...
import {
  commScheduleRulesTable, commTasksTable, eventsTable, usersTable, employeesTable,
  eventBandInvitesTable, eventLineupTable, bandsTable,
  eventStaffSlotsTable, staffRoleTypesTable,
  eventTicketRequestsTable, eventSignupsTable,
} from "@workspace/db";
import { eq, desc, and, lt, inArray, gte, lte, isNotNull, ne, sql } from "drizzle-orm";
...
// ── Auto-emails feed ──────────────────────────────────────────────────────────
// Returns all system-generated automated emails (upcoming + recently sent)
// so the desk can see what will fire automatically without being surprised.
router.get("/comm-schedule/auto-emails", async (req, res) => {
  ...
  try {
    ...
    // ── 1. Band 3-day reminders (one per lineup slot) ─────────────────────────
    ...
    // ── 2. Staff 7-day and 1-day reminders (one per staff slot) ──────────────
    ...
    // ── 5. Debrief nudges (one per event, fires on event end) ─────────────────
    ...
    res.json(entries.map(e => ({ ...e, scheduledDate: e.scheduledDate.toISOString() })));
  } catch (err) {
    console.error("auto-emails error:", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
